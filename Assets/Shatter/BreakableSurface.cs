...
public class BreakableSurface : MonoBehaviour {
		...
		public void Reload() {
			...
			Mesh centeredMesh = CenterMesh(mesh, out meshDisplacement);

			Debug.Log(meshDisplacement);
		...
		public static Mesh CenterMesh(Mesh mesh, out Vector3 displacement)
		{
			...
			if (area > MinBreakArea) {
				...
				for (int i = 0; i < sites.Length; i++) {
					...
					if (clipped.Count > 0) {
						...
						newGo.transform.localPosition = transform.localPosition + meshDisplacement;
